  ...
  checkEmail(): AsyncValidatorFn {
    ...
    return (control: AbstractControl): Observable<{ emailTaken: boolean } | null> => {
      ...
      return timer(debounceMs).pipe(
        switchMap(() =>
          this.http.post<{ available: boolean }>(`${this.apiUrl}/api/users/checkEmail`, { email: control.value }).pipe(
    ...
    return (control: AbstractControl): Observable<{ usernameTaken: boolean } | null> => {
      ...
      return timer(debounceMs).pipe(
        switchMap(() =>
          this.http.post<{ available: boolean }>(`${this.apiUrl}/api/users/checkUsername`, { username: control.value }).pipe(
